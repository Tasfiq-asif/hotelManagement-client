...
const RoomDetails = () => {
  ...
  const [reviewForm, setReviewForm] = useState({ name: '', rating: 1, comment: '' });
  ...
  useEffect(() => {
    const fetchRoomDetails = async () => {
      try {
        const roomResponse = await axios.get(`http://localhost:3000/rooms/${id}`);
        const reviewsResponse = await axios.get(`http://localhost:3000/rooms/${id}/reviews`);
  ...
  const handleBookingSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:3000/rooms/${id}/book`, bookingForm);
  ...
  if (!room) return <div><Spinner></Spinner></div>;
  return (
    <div className="p-8">
      <h2 className="text-3xl font-bold">{room.roomDescription}</h2>
      <img src={room.roomImages[0]} alt="Room" className="w-full h-80 object-cover my-4" />
      ...
      <div className="mt-4">
        ...
        <button onClick={() => setShowReviewPopup(true)} className="btn btn-secondary mt-4">Add Review</button>
      </div>

      <button onClick={() => setShowBookingForm(true)} className="btn btn-primary mt-8">Book Now</button>
      ...
      {showReviewPopup && (
        <div className="modal modal-open">
          <div className="modal-box">
            ...
            <textarea placeholder="Comment" className="textarea textarea-bordered w-full mb-2"
              value={reviewForm.comment} onChange={(e) => setReviewForm({ ...reviewForm, comment: e.target.value })}
            />
            <button onClick={handleReviewSubmit} className="btn btn-primary w-full">Submit</button>
          </div>
        </div>
      )}
          ...
          <div className="modal-box">
            ...
            <form onSubmit={handleBookingSubmit}>
              ...
              <DatePicker
                ...
                onChange={(date) => setBookingForm({ ...bookingForm, checkInDate: date })}
                className="input input-bordered w-full mb-2"
              />
              <label>Check-Out Date</label>
              <DatePicker
                selected={bookingForm.checkOutDate}
                onChange={(date) => setBookingForm({ ...bookingForm, checkOutDate: date })}
                className="input input-bordered w-full mb-2"
              />
              <input type="text" placeholder="Your Name" className="input input-bordered w-full mb-2"
      ...
      {bookingDetails && (
        <div className="mt-8 p-6 bg-gray-100 rounded-lg shadow-md">
          ...
          <p>Check-In: {new Date(bookingDetails.checkInDate).toLocaleDateString()}</p>
          <p>Check-Out: {new Date(bookingDetails.checkOutDate).toLocaleDateString()}</p>
